...
export default function Profile({ user, onLogout }) {
  ...
  return (
    <div className="profile-container">
        ...
        <div className="profile-details">          <p><strong>Username:</strong> {user.name}</p>
          <p><strong>Email:</strong> {user.email}</p>
          <p><strong>Weight:</strong> {user.preferences?.weight} kg</p>
          <p><strong>Height:</strong> {user.preferences?.height} cm</p>
          <p><strong>Date of Birth:</strong> {user.preferences?.age ? new Date(user.preferences.age).toLocaleDateString() : 'Not specified'}</p>
